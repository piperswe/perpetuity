...
lockfile ~/process-$march.lock
trap "rm -f ~/process-$march.lock" EXIT

mkdir -p ~/$incoming_dir perpetuity/$march/{pool,dists/trixie/core/binary-i386}
pushd perpetuity/$march
find ~/$incoming_dir -name '*.deb' -exec cp '{}' pool/ \;
find ~/$incoming_dir -name '*.udeb' -exec cp '{}' pool/ \;
find ~/$incoming_dir -type f -delete
